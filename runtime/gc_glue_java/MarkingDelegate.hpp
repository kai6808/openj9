...
public:
	void dumpObjectCounter(omrobjectptr_t objectPtr, J9Class *clazz, bool compressObjectReferences, MM_EnvironmentBase *env);
	MM_MarkingDelegate()
		: _omrVM(NULL)
		, _extensions(NULL)
		, _markingScheme(NULL)
		...
		if (isDynamicClassUnloadingEnabled()) {
			...
			if ((NULL != objectScanner) && objectScanner->isHeadObjectScanner()) {
				...
				if (_markMap->atomicSetBit(clazz->classObject)) {
					...
					dumpObjectCounter(objectPtr, clazz, env->compressObjectReferences(), env);
				}
			}
		}
